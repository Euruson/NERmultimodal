...
import gensim
# from word2vecReader import pythoWord2Vec
import codecs
# import cPickle
...
def load_sentence(IMAGEID, tweet_data_dir, train_name, dev_name, test_name):
    '''
    read the word from doc, and build sentence. every line contain a word
    and it's tagevery sentence is split with a empty line. every sentence
    begain with an "IMGID:num"
    '''
    # IMAGEID = 'IMGID'
    img_id = []
    sentences = []
    ...
    for fname in (train_name, dev_name, test_name):
        ...
        with open(
                os.path.join(tweet_data_dir, fname), 'r',
            ...
            for line in file:
                ...
                if line == '':
                    ...
                    if IMAGEID in line:
                        ...
                        word_maxlen = max(word_maxlen, len(
                            str(line.split()[0])))
    ...
    return [
        datasplit, sentences, img_to_feature, sent_maxlen, word_maxlen,
        num_sentence
    ]
...
def load_word_matrix(vocabulary, size=200):
    ...
    print("Load_word_matrix")
    # model = word2vec.Word2Vec.load('../40Wtweet_200dim.model')
    model = gensim.models.KeyedVectors.load_word2vec_format(
        './word2vec/w2v.model', binary=False)
    for word, i in vocabulary.items():
        try:
            word_matrix[i] = model[word.lower().encode('utf8')]
        except KeyError:
            # if a word is not include in the vocabulary, it's word embedding
            # will be set by random.
...
def vocab_bulid(sentences):
    ...
    return [
        id_to_vocb, vocb, vocb_inv, vocb_char, vocb_inv_char, labelVoc,
        labelVoc_inv
    ]
...
def label_index(labels_counts):
    """
    the input is the output of Counter. This function defines
    the (label, index) pair, and it cast our datasets label to
    the definition (label, index) pair.
    """
...
def pad_sequence(sentences,
                 ...
                 sent_maxlen=35):
    """
    This function is used to pad the word into the same length,the word length
    is set to 30.
    Moreover, it also pad each sentence into the same length, the length is
    set to 35.
    """
    print(sent_maxlen)
    x = []
    ...
    for sentence in sentences:
        ...
        for i in range(len(s_c_pad)):
            s_pad[sent_maxlen - len(s_c_pad) + i, :len(s_c_pad[i])] = s_c_pad[
                i]
        x_c.append(s_pad)
...
def load_data():
    ...
    #   sent_maxlen, word_maxlen, num_sentence, vocb, vocb_char,
    #   labelVoc = cPickle.load(open(vocabFileName))
    # else:
    print('Calculating vocabulary...')
    (datasplit, sentences, img_to_feature,
     sent_maxlen, word_maxlen, num_sentence) = load_sentence(
         IMAGEID, tweet_data_dir, 'train', 'dev', 'test')

    id_to_vocb, vocb, vocb_inv, vocb_char, vocb_inv_char, \
        labelVoc, labelVoc_inv = vocab_bulid(sentences)

    word_matrix = load_word_matrix(vocb, size=200)

    x, x_c, img_x, y = pad_sequence(
        sentences,
        img_to_feature,
        vocb,
        vocb_char,
    ...
    return [
        id_to_vocb, word_matrix, sentences, datasplit, x, x_c, img_x, y,
        num_sentence, vocb, vocb_char, labelVoc
    ]
