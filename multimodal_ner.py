...
if __name__ == '__main__':
    ...
    tweet_new_resize = TimeDistributed(
    ...
    acc_test, f1_test, p_test, r_test = evaluate(pre_test_label_index,
                                                 label_test, te_x, labelVoc,
                                                 sent_maxlen, id_to_vocb)
    pre_test_label_index_2 = pre_test_label_index.reshape(
        len(label_test) * sent_maxlen)
    print('----------')
    print('##test##, evaluate:'
          'F1:', f1_test, 'precision:', p_test, 'recall:', r_test)

    # evaluate each class
    for class_type in ('PER', 'LOC', 'ORG', 'OTHER'):
        f1_t_cl, p_t_cl, r_t_cl = evaluate_each_class(
            pre_test_label_index, label_test, te_x, labelVoc, sent_maxlen,
            id_to_vocb, class_type)
        print('class type:', class_type, 'F1:', f1_t_cl, 'precision:', p_t_cl,
              'recall:', r_t_cl)
